...
updateFirst key seq = insertMap key digit (digit:renormalize qes)
  where renormalize = mapEveryOther (`div` 3) . map fromParity
        ...
        qes = reverse seq-- file: ch12/Barcode.hs
...
firstDigit = snd
  ...
  where parityBit (Even _) = Zero
        parityBit (Odd _) = One-- file: ch12/Barcode.hs
finalDigits :: [[Parity Digit]] -> ParityMap
finalDigits = foldl' incorporateDigits (M.singleton 0 [])
            . mapEveryOther (map (fmap (*3)))-- file: ch12/Barcode.hs
incorporateDigits :: ParityMap -> [Parity Digit] -> ParityMap
incorporateDigits old digits = foldl' (useDigit old) M.empty digits-- file: ch12/Barcode.hs
useDigit :: ParityMap -> ParityMap -> Parity Digit -> ParityMap
useDigit old new digit =
    new `M.union` M.foldWithKey (updateMap digit) M.empty old-- file: ch12/Barcode.hs
...
insertMap key digit val m = val `seq` M.insert key' val m
    where key' = (key + digit) `mod` 10-- file: ch12/Barcode.hs
type DigitMap = Map Digit
type ParityMap = Map (Parity Digit)-- file: ch12/Barcode.hs
type Map a = M.Map Digit [a]-- file: ch12/Barcode.hs
candidateDigits rle
    | any null match = []
  ...
  where match = map bestLeft left ++ map bestRight right
        ...
        runLengths = map fst rle-- file: ch12/Barcode.hs
candidateDigits :: RunLength Bit -> [[Parity Digit]]
candidateDigits ((_, One):_) = []
candidateDigits rle | length rle < 59 = []-- file: ch12/Barcode.hs
chunkWith :: ([a] -> ([a], [a])) -> [a] -> [[a]]
chunkWith _ [] = []
...
chunksOf :: Int -> [a] -> [[a]]
chunksOf n = chunkWith (splitAt n)-- file: ch12/Barcode.hs
data AltParity a = AltEven {fromAltParity :: a}
                 | AltOdd  {fromAltParity :: a}
                 | AltNone {fromAltParity :: a}
                   deriving (Show)-- file: ch12/Barcode.hs
type Digit = Word8
...
bestRight = map None . bestScores rightSRL-- file: ch12/Barcode.hs
on :: (a -> a -> b) -> (c -> a) -> c -> c -> b
on f g x y = g x `f` g y

compareWithoutParity = compare `on` fromParity-- file: ch12/Barcode.hs
...
instance Functor Parity where
    fmap = parityMap-- file: ch12/Barcode.hs
-- our original
...
[(distance d (scaleToOne ps), n) | d <- srl, n <- digits]-- file: ch12/Barcode.hs
bestScores :: ScoreTable -> [Run] -> [(Score, Digit)]
bestScores srl ps = take 3 . sort $ scores
    where scores = zip [distance d (scaleToOne ps) | d <- srl] digits
          digits = [0..9]-- file: ch12/Barcode.hs
distance :: [Score] -> [Score] -> Score
distance a b = sum . map abs $ zipWith (-) a b-- file: ch12/Barcode.hs
...
paritySRL = asSRL parityList-- file: ch12/Barcode.hs
runLengths :: Eq a => [a] -> [Run]
runLengths = map fst . runLength-- file: ch12/Barcode.hs
type Run = Int
type RunLength a = [(Run, a)]

runLength :: Eq a => [a] -> RunLength a
runLength = map rle . group
    where rle xs = (length xs, head xs)-- file: ch12/Barcode.hs
data Bit = Zero | One
           deriving (Eq, Show)
...
threshold n a = binary <$> a
    where binary i | i < pivot  = Zero
          ...
          choose f = foldA1 $ \x y -> if f x y then x else y-- file: ch12/Barcode.hs
type Greymap = Array (Int,Int) Pixel

pixmapToGreymap :: Pixmap -> Greymap
pixmapToGreymap = fmap luminance-- file: ch12/Barcode.hs
luminance :: (Pixel, Pixel, Pixel) -> Pixel
luminance (r,g,b) = round (r' * 0.30 + g' * 0.59 + b' * 0.11)
    where r' = fromIntegral r
          g' = fromIntegral g
          b' = fromIntegral b-- file: ch12/Barcode.hs
...
parseTimes n p = p ==> \x -> (x:) <$> parseTimes (n-1) p-- file: ch12/Barcode.hs
type Pixel = Word8
type RGB = (Pixel, Pixel, Pixel)

type Pixmap = Array (Int,Int) RGB-- file: ch12/Barcode.hs
...
outerGuard = "101"
centerGuard = "01010"-- file: ch12/Barcode.hs
...
foldA1 f a = foldA f (a ! fst (bounds a)) a-- file: ch12/Barcode.hs
...
parityCodes = listToArray parityList-- file: ch12/Barcode.hs
...
import Parse                    -- from chapter 11-- file: ch12/Barcode.hs
