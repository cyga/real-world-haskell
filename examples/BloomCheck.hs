...
prop_suggestions_sane =
  ...
  where sane (bits,hashes) = bits > 0 && bits < maxBound && hashes > 0
        maxWord32 = maxBound :: Word32-- file: examples/BloomCheck.hs
prop_suggest_try2 =
    forAll falsePositive $ \errRate ->
  ...
  forAll falsePositive $ \errRate ->
    forAll (choose (1,maxBound :: Word32)) $ \cap ->
      case B.suggestSizing (fromIntegral cap) errRate of
        Left err -> False
        Right (bits,hashes) -> bits > 0 && bits < maxBound && hashes > 0-- file: examples/BloomCheck.hs
...
instance Arbitrary Word32 where
    ...
    coarbitrary = integralCoarbitrary-- file: examples/BloomCheck.hs
...
instance Arbitrary Strict.ByteString where
    ...
    coarbitrary = coarbitrary . Strict.unpack-- file: examples/BloomCheck.hs
prop_all_present _ xs =
    forAll falsePositive $ \errRate ->
      B.easyList errRate xs =~> \filt ->
        all (`B.elem` filt) xs-- file: examples/BloomCheck.hs
...
prop_one_present _ elt =
    forAll falsePositive $ \errRate ->
      B.easyList errRate [elt] =~> \filt ->
        elt `B.elem` filt-- file: examples/BloomCheck.hs
