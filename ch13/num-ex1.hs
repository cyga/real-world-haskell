...
prettyShow (BinaryArith op a b) =
    let (pa, pao) = simpleParen a
        (pb, pbo) = simpleParen b
        pop = op2str op
        par = if pao /= Nothing && unJust pao < op then "("++pa++")" else pa
        pbr = if pbo /= Nothing && unJust pbo < op then "("++pb++")" else pb
        in par ++ pop ++ pbr
...
simpleParen :: (Show a, Num a) => SymbolicManip a -> (String, Maybe Op)
simpleParen (Number x) = (prettyShow (Number x), Nothing)
simpleParen (Symbol x) = (prettyShow (Symbol x), Nothing)
simpleParen x@(BinaryArith op _ _) = (prettyShow x, Just op)
-- let's assume that unary operator comes with parenths
simpleParen x@(UnaryArith _ _) = (prettyShow x, Nothing)
