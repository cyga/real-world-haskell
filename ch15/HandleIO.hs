...
tidyHello path = do
  safeHello path
  liftIO (removeFile path)-- file: ch15/HandleIO.hs
import Control.Monad.Trans (MonadIO(..))

instance MonadIO HandleIO where
    liftIO = HandleIO-- file: ch15/HandleIO.hs
safeHello :: FilePath -> HandleIO ()
safeHello path = do
  h <- openFile path WriteMode
  hPutStrLn h "hello world"
  hClose h-- file: ch15/HandleIO.hs
...
hPutStrLn h s = HandleIO (System.IO.hPutStrLn h s)-- file: ch15/HandleIO.hs
newtype HandleIO a = HandleIO { runHandleIO :: IO a }
    deriving (Monad)-- file: ch15/HandleIO.hs
