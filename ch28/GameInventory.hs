...
tryBogusSale = do
  ...
  bogusSale Wand 5 alice bob-- file: ch28/GameInventory.hs
...
consistentBalance players = do
  ...
  where totalBalance   = foldM addBalance 0 players
        addBalance a b = (a+) `liftM` readTVar (balance b)-- file: ch28/GameInventory.hs
...
populateWorld = sequence [ newPlayer 20 20 [Wand, Banjo],
                           newPlayer 10 12 [Scroll] ]-- file: ch28/GameInventory.hs
...
bogusSale item price buyer seller = do
  ...
  bogusTransfer price (balance buyer) (balance seller)-- file: ch28/GameInventory.hs
maybeM :: MonadPlus m => m a -> m (Maybe a)
maybeM m = (Just `liftM` m) `mplus` return Nothing-- file: ch28/GameInventory.hs
...
shoppingList list buyer seller = maybeSTM . msum $ map sellOne list
    where sellOne this@(item,price) = do
            ...
            return this-- file: ch28/GameInventory.hs
maybeSTM :: STM a -> STM (Maybe a)
maybeSTM m = (Just `liftM` m) `orElse` return Nothing-- file: ch28/GameInventory.hs
...
transfer qty fromBal toBal = do
  ...
  readTVar toBal >>= writeTVar toBal . (qty +)-- file: ch28/GameInventory.hs
...
giveItem item fromInv toInv = do
  ...
  case removeInv item fromList of
    ...
    Just newList -> do
      ...
      readTVar toInv >>= writeTVar toInv . (item :)-- file: ch28/GameInventory.hs
...
maybeSellItem item price buyer seller = do
  ...
  if given
    then do
      ...
      return True
    else return False-- file: ch28/GameInventory.hs
basicTransfer :: Gold -> Balance -> Balance -> STM ()
maybeGiveItem :: Item -> Inventory -> Inventory -> STM Bool-- file: ch28/GameInventory.hs
maybeGiveItem item fromInv toInv = do
  ...
  case removeInv item fromList of
    ...
    Just newList -> do
      ...
      return True-- file: ch28/GameInventory.hs
removeInv :: Eq a => a -> [a] -> Maybe [a]
removeInv x xs =
    case takeWhile (/= x) xs of
      (_:ys) -> Just ys
      []     -> Nothing-- file: ch28/GameInventory.hs
transferTest = do
  alice <- newTVar (12 :: Gold)
  bob   <- newTVar 4
  basicTransfer 3 alice bob
  liftM2 (,) (readTVar alice) (readTVar bob)-- file: ch28/GameInventory.hs
basicTransfer qty fromBal toBal = do
  fromQty <- readTVar fromBal
  toQty   <- readTVar toBal
  writeTVar fromBal (fromQty - qty)
  writeTVar toBal   (toQty + qty)-- file: ch28/GameInventory.hs
