...
options = [ Option ['h'] ["help"] (NoArg Help)
            ...
            Option ['n'] []       (ReqArg (\s -> N (read s)) "N")
                   "Number of concurrent connections (default 16)" ]-- file: ch28/Check.hs
...
parseArgs = do
  ...
  where
    ...
    help       = dump info                  >> exitWith ExitSuccess-- file: ch28/Check.hs
...
extractLinks = concatMap uris . B.lines
  where uris s      = filter looksOkay (B.splitWith isDelim s)
        ...
        http        = B.pack "http:"-- file: ch28/Check.hs
...
sendJobs js = do
    c <- gets linkQueue
    liftIO . atomically $ mapM_ (writeTChan c . Check) js-- file: ch28/Check.hs
...
execJob = execStateT . runJob-- file: ch28/Check.hs
  ...
  where
    ...
    checkOne url = case parseURI url of
        ...
        where report s = atomically $ do
                           ...
                           writeTChan badLinks (url ++ " " ++ s)-- file: ch28/Check.hs
...
embedMaybe err = maybe (throwError err) return-- file: ch28/Check.hs
...
getHead uri = simpleHTTP Request { rqURI = uri,
                                   rqMethod = HEAD,
                                   rqHeaders = [],
                                   rqBody = "" }-- file: ch28/Check.hs
waitFor :: TVar Int -> IO ()
waitFor alive = atomically $ do
  count <- readTVar alive
  check (count == 0)-- file: ch28/Check.hs
writeBadLinks :: TChan String -> IO ()
writeBadLinks c =
  forever $
    atomically (readTChan c) >>= putStrLn >> hFlush stdout-- file: ch28/Check.hs
...
forkTimes k alive act =
  replicateM_ k . forkIO $
    act
    `finally`
    (atomically $ modifyTVar_ alive (subtract 1))-- file: ch28/Check.hs
main :: IO ()
main = do
    (files,k) <- parseArgs
    let n = length files
